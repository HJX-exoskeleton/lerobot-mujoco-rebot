...
from rebot_aerohand_right_act_sim.policy import make_policy
...
def _save_metrics(
    output, history: list[float], *, best_step: int | None = None
) -> None:
    average = []
...
def main() -> None:
    ...
    parser.add_argument("--hand-contact", action=argparse.BooleanOptionalAction, default=None)
    ...
    use_hand_contact = bool(
        multimodal.get("use_hand_contact", False)
        if args.hand_contact is None
        else args.hand_contact
    )
    ...
    validate_processing_spec(root, hand_contact_processing)
    ...
    history: list[float] = []
    avg_window: deque[float] = deque(maxlen=100)
    best_avg = float("inf")
    ...
    for step in progress:
        ...
        if save_freq > 0 and step % save_freq == 0 and step < steps:
            ...
            if use_hand_contact:
                write_or_validate_processing_spec(checkpoint, hand_contact_processing)
    ...
    if use_hand_contact:
        write_or_validate_processing_spec(final_step_ckpt, hand_contact_processing)

    policy.eval()
    final_checkpoint = output / "pretrained_model"
    policy.save_pretrained(final_checkpoint)
    if use_hand_contact:
        write_or_validate_processing_spec(final_checkpoint, hand_contact_processing)

    # Persist the checkpoint with the best 100-step average loss.  On small
    # datasets the final weights often overfit; the best checkpoint is the
    # one to deploy.
    if best_state_dict is not None:
        policy.load_state_dict(best_state_dict)
        best_checkpoint = output / "best_model"
        policy.save_pretrained(best_checkpoint)
        if use_hand_contact:
            write_or_validate_processing_spec(best_checkpoint, hand_contact_processing)
        best_state_dict = None

    _save_metrics(output, history, best_step=best_step)
    run = {
        "config": raw,
        "use_imu": use_imu,
        "use_hand_contact": use_hand_contact,
    ...
    if best_step is not None:
        print(
            f"Best avg100 loss {best_avg:.5f} at step {best_step} -> "
            f"{output / 'best_model'}"
        )
