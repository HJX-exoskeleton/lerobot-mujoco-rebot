...
from rebot_aerohand_right_act_sim.environment import SimAeroHandACTEnvironment
from rebot_aerohand_right_act_sim.timing import WallClockRate
from rebot_aerohand_right_act_sim.visualization import ReplaySensorVisualizer
...
def main() -> None:
    ...
    parser.add_argument(
        "--no-sensors",
        action="store_true",
        help="Disable the synchronized IMU/hand-contact panel.",
    )
    parser.add_argument(
        "--speed",
        type=float,
            ...
            if frame_index % render_every == 0:
                ...
                if not args.no_sensors:
                    sensor_panel = sensor_visualizer.render(
                        ...
                        batch["sensor.hand_contact"][0].numpy(),
                        batch["sensor.hand_feedback"][0].numpy(),
                        frame_index=frame_index,
                        timestamp=float(batch["timestamp"][0]),
                    )
                status_lines = (
                    "REPLAY",
                    f"episode={args.episode}  frame={frame_index}  "
                    f"t={float(batch['timestamp'][0]):.2f}s",
                )
                env.render(
                    sensor_panel=sensor_panel,
                    status_lines=status_lines,
                    show_camera_overlays=not args.no_sensors,
                )
            frame_index += 1
            rate.wait()
    finally:
