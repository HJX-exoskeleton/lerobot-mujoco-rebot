...
class SimAeroHandACTDatasetWriter:
    def __init__(
        self,
        *,
        repo_id: str,
        ...
        self.dataset = (
            ...
            if info.is_file()
                ...
                repo_id=repo_id,
                root=self.root,
                robot_type="rebot_aerohand_right_sim",
                fps=fps,
        ...
        write_or_validate_processing_spec(self.root, hand_contact_processing)
    ...
    def discard_episode(self) -> None:
        # Flush pending asynchronous image writes first: clear_episode_buffer
        # removes the episode image directory, and writer threads still
        # flushing into it would otherwise raise "Directory not empty".
        writer = getattr(self.dataset, "image_writer", None)
        ...
        for _ in range(3):
            ...
            raise RuntimeError("failed to clear episode buffer after retries")
        self.frames_in_buffer = 0
