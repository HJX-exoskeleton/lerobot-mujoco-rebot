...
Run from the mujoco workspace
-----------------------------
python3 aerohand_right_control/aerohand_right_cv_control_sim.py

Run with explicit right-hand model and Linux low-latency camera settings
------------------------------------------------------------------------
python3 aerohand_right_control/aerohand_right_cv_control_sim.py --start-enabled
...
Linux OpenGL modes
------------------
python3 aerohand_right_control/aerohand_right_cv_control_sim.py --gl-mode software
python3 aerohand_right_control/aerohand_right_cv_control_sim.py --gl-mode hardware
...
LOCAL_MODEL = SCRIPT_DIR / "mujoco_xml" / "rebot_arm_right_hand.xml"
SHARED_MODEL = (
    WORKSPACE_DIR / "asset_rebot_aerohand_right" /
    "mujoco_xml" / "rebot_arm_right_hand.xml"
)
# Future model development can be placed directly under
# aerohand_right_control/mujoco_xml without changing this script.
DEFAULT_MODEL = LOCAL_MODEL if LOCAL_MODEL.is_file() else SHARED_MODEL
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help=(
            "MuJoCo XML path. Relative paths are searched from the current "
            "directory first, then from aerohand_right_control/."
        ),
    )
...
def resolve_model_path(path: Path) -> Path:
    """Resolve an explicit model path without depending on the launch cwd."""
