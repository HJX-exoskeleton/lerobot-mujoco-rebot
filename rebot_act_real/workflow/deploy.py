#!/usr/bin/env python3
"""Deploy a trained LeRobot ACT checkpoint on the real reBot arm.
...
Shadow mode reads real cameras/joints and runs inference without motor commands::
    ...
    python -m rebot_act_real.workflow.deploy \
      ...
      --shadow --n-action-steps 10 --visualize

Real execution requires ``--execute`` and DEPLOY confirmation. ACT outputs are
already in the real dataset action space: six absolute joint targets plus the
Damiao gripper target, all in radians.
"""
...
def _load_policy(
    ...
    *,
    ...
    if n_action_steps is not None and n_action_steps <= 0:
        raise ValueError("--n-action-steps必须大于0")
    ...
    if temporal_ensemble:
        ...
        config.n_action_steps = int(
            n_action_steps if n_action_steps is not None else config.n_action_steps
        )
    ...
    if is_multimodal_checkpoint(checkpoint):
        ...
        policy = ACTPolicy.from_pretrained(
            checkpoint, config=config, dataset_stats=metadata.stats, local_files_only=True
        ).to(device).eval()
    policy.reset()
...
def build_argparser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="ACT reBot真机策略部署")
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="动作队列每次推理执行步数；省略时使用checkpoint配置",
    )
    parser.add_argument(
        "--temporal-ensemble",
        action="store_true",
        help="启用LeRobot ACT原生逐步时间集合，并强制n_action_steps=1",
    )
    parser.add_argument(
    ...
    parser.add_argument("--gripper-kp", type=float, default=1.2)
    parser.add_argument("--gripper-kd", type=float, default=0.055)
        ...
        if args.record:
            recorder = PolicyRunRecorder(
                ...
                extra_metadata={
                    ...
                    "control_layer": "position_velocity",
                    "execute": execute,
                },
            )
        ...
        with torch.inference_mode():
            while not stop and (args.max_steps <= 0 or step < args.max_steps):
                ...
                if recorder is not None:
                    ...
                    record_sample["sensor_frame_ids"] = np.asarray(
                        [
                    ...
                    record_sample["sensor_timestamp_s"] = np.asarray(
                        [
