...
REBOT_SMOLVLA_ROOT = CURRENT_DIR.parent
SDK_PATH = REBOT_SMOLVLA_ROOT / "STservo_sdk"

if not (SDK_PATH / "__init__.py").is_file():
    raise FileNotFoundError(f"STservo_sdk 不存在或不完整: {SDK_PATH}")

# SDK 自身部分文件仍使用 ``from stservo_def import *`` 形式的旧式绝对导入，
# 所以包目录和包的父目录都必须加入搜索路径。
for import_path in (REBOT_SMOLVLA_ROOT, SDK_PATH):
