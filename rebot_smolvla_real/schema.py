"""Canonical LeRobot contract for reBot multimodal demonstrations.

Only ``observation.*`` and ``action`` are policy features.  Auxiliary sensors
deliberately use the ``sensor.*`` namespace: LeRobot preserves them in the
dataset but ``dataset_to_policy_features`` does not feed them to SmolVLA.
"""
...
def validate_frame(frame: Mapping[str, object], *, require_auxiliary: bool = True) -> None:
    ...
    if missing:
        raise KeyError(f"SmolVLA real frame is missing features: {missing}")
