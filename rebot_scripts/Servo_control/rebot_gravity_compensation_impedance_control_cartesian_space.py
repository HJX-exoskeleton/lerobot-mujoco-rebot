...
# 与关节空间阻抗验证脚本保持一致。
TORQUE_LIMITS = np.array([10.0, 10.0, 10.0, 6.0, 6.0, 6.0])
...
# 姿态保持项。第三轴通常对应底座 yaw，对关节 1 恢复最关键。
K_ROT_BASE = np.array([0.8, 0.8, 9])
D_ROT_BASE = np.array([0.12, 0.12, 0.9])
ENABLE_ORIENTATION_HOLD = True  # False

# 在线整体缩放系数，通过 [ / ] 调整。
CARTESIAN_STIFFNESS_SCALE = 1.0

# 笛卡尔力/力矩与叠加后关节力矩限幅。
MAX_CART_FORCE_N = 15.0
MAX_CART_TORQUE_NM = 6.0
# TASK_TORQUE_LIMITS = np.array([4.0, 4.0, 4.0, 2.0, 2.0, 2.0])
TASK_TORQUE_LIMITS = np.array([10.0, 10.0, 10.0, 6.0, 6.0, 6.0])

# 速度估计滤波，用于末端阻尼与打印。
QD_FILTER_ALPHA = 0.25

# 关节 1 辅助保持。只在 CARTESIAN_HOLD 生效，用来补偿纯笛卡尔姿态项
# 经 Jacobian 映射到底座关节后恢复力不足的问题。
JOINT1_HOLD_ENABLE = True
JOINT1_HOLD_KP = 4.0
JOINT1_HOLD_KD = 0.35
JOINT1_HOLD_TORQUE_LIMIT = 10.0
...
def print_status(controller=None):
    ...
    print(f"JOINT1_HOLD_ENABLE       = {JOINT1_HOLD_ENABLE}")
    print(f"JOINT1_HOLD_KP/KD/LIMIT  = {JOINT1_HOLD_KP}, {JOINT1_HOLD_KD}, {JOINT1_HOLD_TORQUE_LIMIT}")
    print("=" * 76 + "\n")
...
class CartesianImpedanceController:
    ...
    def _compute_joint1_hold_torque(self, q, qd_est, n):
        if not JOINT1_HOLD_ENABLE or self.q_ref is None or n < 1:
            return np.zeros(n)

        tau = np.zeros(n)
        q1_err = float(self.q_ref[0] - q[0])
        qd1 = float(qd_est[0]) if len(qd_est) > 0 else 0.0
        tau[0] = JOINT1_HOLD_KP * q1_err - JOINT1_HOLD_KD * qd1
        tau[0] = np.clip(tau[0], -JOINT1_HOLD_TORQUE_LIMIT, JOINT1_HOLD_TORQUE_LIMIT)
        return tau
    ...
    def __call__(self, arm: RobotArm, dt: float) -> None:
        ...
        try:
            ...
            if self.mode == "FREE_DRAG":
                ...
                tau_joint1_hold = self._compute_joint1_hold_torque(q, qd_est, n)
                tau_cmd = np.clip(
                    tau_g_safe + tau_task + tau_joint1_hold,
                    -TORQUE_LIMITS[:n],
                    TORQUE_LIMITS[:n],
                )
            ...
            if self.counter % 50 == 0:
                ...
                if self.mode == "CARTESIAN_HOLD":
                    ...
                    print(
                        ...
                        + f" N*m, tau_j1_hold={tau_joint1_hold[0]:+.2f} N*m"
                    )
...
def main() -> None:
    ...
    print(f"  JOINT1_HOLD    = enable={JOINT1_HOLD_ENABLE}, kp={JOINT1_HOLD_KP}, "
          f"kd={JOINT1_HOLD_KD}, limit={JOINT1_HOLD_TORQUE_LIMIT}")
    print("-" * 76)
    print("[操作说明]")
