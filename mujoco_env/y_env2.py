...
class SimpleEnv2:
    ...
    def reset(self, seed = None):
        ...
        # A bent reBot seed avoids the singular straight-arm zero configuration.
        q_init = np.array([0.0, -1.0, -1.0, 0.0, 0.0, 0.0])
        q_zero,ik_err_stack,ik_info = solve_ik(
            ...
            # gripper points down toward the tabletop (local +Z faces forward).
            R_trgt       = rpy2r(np.deg2rad([0.0, 90.0, 0.0])),
        )
    ...
    def step(self, action):
        ...
        gripper_cmd = self.gripper_open + action[-1] * (self.gripper_closed - self.gripper_open)
        self.compute_q = q
    ...
    def get_joint_state(self):
        ...
        gripper_cmd = float(gripper[0] < (self.gripper_open + self.gripper_closed) / 2)
    ...
    def check_success(self):
        ...
        Check whether the target mug remains on the plate with the gripper open.
        '''
        ...
        instantaneous_success = (
            ...
            and self.env.get_qpos_joint(self.gripper_joint_name)[0] > 0.04
        )
        self.success_hold_count = self.success_hold_count + 1 if instantaneous_success else 0
        return self.success_hold_count >= self.success_hold_steps
